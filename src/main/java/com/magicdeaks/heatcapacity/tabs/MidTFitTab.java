...
public class MidTFitTab extends JPanel implements PropertyChangeListener {
    ...
    private void buildLayout() {
        ...
        topConfigPanel.add(new JLabel("Start Power:"));
        topConfigPanel.add(startPowerField);
        topConfigPanel.add(new JLabel("Increment:"));
        topConfigPanel.add(incrField);
            ...
            if (Arrays.stream(fitResults).noneMatch(Objects::isNull)) {
                ...
                if (selectedIdx >= 0 && selectedIdx < fitResults.length) {
                    try {
                        ...
                        double[][] pairs =
                                new double[fitResults[selectedIdx].coefficients().length][];
                        for (int j = 0; j < fitResults[selectedIdx].coefficients().length; j++) {
                            pairs[j] =
                                    new double[] {
                                        fitResults[selectedIdx].coefficients()[j], doublePowers[j]
                                    };
                        ...
                        if (rawData != null) {
                            if (rawData.temperatures() != null) {
                                double[][] deviationsSet =
                                        getDeviations(
                                                powers[selectedIdx],
                                                fitResults[selectedIdx].coefficients(),
                                                rawData);

                                currentDeviations.addSeries("Dev-" + selectedIdx, deviationsSet);

                                FitResult[] updatedFits = new FitResult[11];
                                for (int i = 0; i < NUM_FITS; i++) {
                                    updatedFits[i] =
                                            new FitResult(
