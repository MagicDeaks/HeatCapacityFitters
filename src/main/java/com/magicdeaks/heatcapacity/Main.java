...
public class Main {
  public static void main(String[] args) {
    ...
    //   temperatures[i] = i * 0.001 + 0.001;
    // }

    // double[] heatCapacities = Aluminum.calculateBCSHeatCapacities(temperatures);

    // try (FileWriter writer = new FileWriter("output.csv")) {
    //   for (int i = 0; i < 1000; i++) {
    //     writer
    //         .append(String.valueOf(temperatures[i]))
    //         .append(",")
    //         .append(String.valueOf(heatCapacities[i]))
    //         .append("\n");
    //   }
    // } catch (IOException e) {
    //   e.printStackTrace();
    ...
    // DataProcesser.scaleHeatCapacity(DataProcesser.subtractCopper(DataReader.readDAT("data.dat"),
    // 14.42), 9.38, 0.667 + DataProcesser.getMolecularWeight("SmOHCO3"));
    ...
    // double[] highT = Arrays.copyOfRange(data.temperatures(), highIdx,
    // data.temperatures().length);
    // double[] highHC = Arrays.copyOfRange(data.heatCapacities(), highIdx,
    // data.heatCapacities().length);
    ...
    // FitResult lowResult = LmCurveFitter.fit(lowTModel, lowData, lowInitParams, lowFixedParams,
    // lowLowerBounds, lowUpperBounds, 500, 1500);
    // FitResult highResult = LmCurveFitter.fit(highTModel, highData, highInitParams,
    // highFixedParams, highLowerBounds, highUpperBounds, 15000, 45000);
