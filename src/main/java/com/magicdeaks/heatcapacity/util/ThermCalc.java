    ...
    public static double[] calculateEnthalpies(double[] temperatures, double[] heatCapacities) {
        ...
        for (int i = 0; i < temperatures.length; i++) {
            if (i == 0) {
                ...
                enthalpies[i] = (heatCapacities[i] - heatCapacities[i-1]) * (temperatures[i] - temperatures[i-1]) + enthalpies[i-1];
            }
        }
    ...
    public static double[] calculateEntropies(double[] temperatures, double[] heatCapacities) {
        ...
        for (int i = 0; i < temperatures.length; i++) {
            if (i == 0) {
                ...
                entropies[i] = (scaledHeatCapacities[i] - scaledHeatCapacities[i-1]) * (temperatures[i] - temperatures[i-1]) + entropies[i-1];
            }
        }
